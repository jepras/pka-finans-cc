// Genererer session2-code/data/finanskube-2025.sqlite. Alt er syntetisk.
// Kør fra session2-facit: bun run scripts/generer-data.ts
...
const steder: [string, string, string, number][] = [
  ...
  ["600", "HR", "Finans", 0.2],
  ["700", "Fondsadministration", "Finans", 0.85],
...
for (const s of steder) {
  ...
  insNoegle.run(s[0], s[3], Math.round((1 - s[3]) * 100) / 100);
}
...
// Kontroltotaler: hvad Finanskuben selv rapporterer pr. periode
...
for (const r of rows) insTot.run(r.periode, r.total);
...
console.log(`Skrev ${n} transaktioner, total ${total.toLocaleString("da-DK")} kr.`);
db.close();
